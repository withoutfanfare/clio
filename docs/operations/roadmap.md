...
| CLIO-OPS-002 | 1 | Blocked: machines are offline or unreachable by SSH | Install the current release on the Mac mini and MacBook Pro | Resume as soon as each machine is online and reachable. | Each machine checks out the same pushed SHA, runs `macos-install.sh`, reconnects its AI clients, and passes cross-machine write, recall and namespace checks. |
| CLIO-APP-001 | 1 | Planned | Persist Tauri Atlas connection settings | Implement when the desktop app must connect to Atlas when opened from Finder. | Atlas settings persist securely, can be tested in the app, show a useful failure state, and require no shell environment. |
