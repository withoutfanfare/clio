...
- `tests/waypoint_worktree_projection.rs` originated as coverage for Waypoint's
  retired Worktree Ledger projection. It remains valuable generic coverage for
  `source + source_ref` upserts, tag-based recall, archive-not-delete, and
  resume briefs. Keep these storage and retrieval behaviours passing.
